...
class OpdoutController extends Controller
{
    ...
    if($request->ajax()) {
        // $latestIds = PatientLog::select(DB::raw('MAX(id) as id'))->groupBy('patient_id');
        // $query = PatientLog::whereIn('id', $latestIds->pluck('id'));
        // if ($request->doctor_id) {
        //     $query->where('doctor_id', $request->doctor_id);
        // }
        // $appointments = $query->get();
        $appointments = PatientLog::where('type','OPD')->orderBy('id','desc')->get();
    ...
    function opdOutDetails($appointment_id,$patient_id){
        $patients = Patient::where('id',$patient_id)->get();
        $appointments = PatientLog::where('patient_id',$patient_id)->get();
        ...
        $visitsData = Visit::where('patient_id',$patient_id)->get();
        $medicationData = Medication::with('medicineNameData')->where('patient_id',$patient_id)->get();
        $testtypes = TestType::where('status',1)->get();
        $testnames = TestName::where('status',1)->get();
        $labInvestigationData = LabInvestigation::where('patient_id',$patient_id)->get();
        ...
        $admit_lists = AdmitList::with('patientData')->where('patient_id',$patient_id)->orderBy('id','desc')->get();
        return view('backend.admin.modules.opdout.opd-out-details',compact('appointment_id','patients','appointments','medicineCategory','departments','doctorData','visitsData','medicationData','testtypes','testnames','labInvestigationData','ipdAvailBeds','icuAvailBeds','admit_lists'));
    }
    function moveToIpdStatus(Request $request){
        return DB::transaction(function () use ($request) {
            $appointment_date = Carbon::Parse(PatientLog::where('id',$request->appointment_id)->value('appointment_date'));
            $today = date('d-m-Y');
            $interval = $appointment_date->diff($today);
            $days = $interval->format('%d');
            if($days != 0){
                return response()->json(["error_movement"=>"Only today's appointments can be moved"]);
            }
            ...
            if($update){
                ...
                $patient_logs->doctor_id = $request->doctor_id;
                $patient_logs->reference_person = $request->ref_person;
                $patient_logs->current_status = "Moved";
                $patient_logs->description = "Movement OPD to IPD";
                $patient_logs->save();
                //clicked opd appointment mark as Moved to IPD
                PatientLog::where('patient_id',$request->id)->where('id',$request->appointment_id)->update([
                    'admit_id' => $request->id,
                    'status' => 'Moved to IPD'
                ]);
                //other then clicked opd appointment where status is Pending mark as Cancelled
                PatientLog::where('patient_id',$request->id)->where('id','!=',$request->appointment_id)->where('status','Pending')->whereNull('admit_id')->update([
                    'admit_id' => '0000',
                    'status' => 'Cancelled',
                    'description' => 'Sibling appointment Moved to IPD'
                ]);
                ...
                $admit_data->desc = "Patient moved to IPD from OPD";
                $admit_data->save();
    ...
    if($request->ajax()){
            ...
            ->addColumn('visit_date',function($row){
                return $row->visited_date ?? $row->created_at->setTimezone('Asia/Kolkata')->format('d-m-Y'); //fetched through modal relationship
            })
            ->addColumn('doctor',function($row){
    ...
    public function viewOpdOutCharge(Request $request){
        if($request->ajax()){
            $labTestDetails = PaymentBill::where('patient_id',$request->patient_id)->whereNull('admit_id')->get();
            return DataTables::of($labTestDetails)
            ->addColumn('created_at',function($row){
    ...
    public function opdOutVItalSubmit(Request $request){
        ...
        // $optoutCharge->admit_id = $admit_id;
   ...
   public function opdOutVItalDataUpdate(Request $request){
    ...
    public function opdOutAdvanceSubmit(Request $request){
        ...
        // $admit_id = Patient::where('id',$request->patientId)->value('admit_id');i
